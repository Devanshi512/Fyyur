...
db.create_all()
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:admin@localhost:5432/fyyur' # Database is 'fyyur'
...
def search_venues():
  ...
  for venue in venues:
      ...
      data.append({
        "id": venue.id,
        "name": venue.name,
        "no_of_upcoming_shows": no_of_upcoming_shows
      })
...
# Display specific Venue with its ID
...
# Edit/Modify specific Venue with its ID
...
#  Delete specific Venue with its ID
...
# Showing specific Artist with its ID
...
#  Edit/Modify specific Artist via its ID
...
def shows():
    ...
    for show in shows:
        # Allocate the show based on Artist ID and Venue ID
